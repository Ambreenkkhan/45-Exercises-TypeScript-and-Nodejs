...
while (invitationList.length > 2) {
  ...
  console.log(
    `Dear Mr. ${notInvited} sorry! We don't have a big table, We can't invite you for dinner.\n`
  );
}
// • Print a message to each of the two people still on your list, letting them know they’re still invited.
for (let i = 0; i < invitationList.length; i++) {
  console.log(
    `Dear Mr.${invitationList[i]}: You are still invited for dinner.\n`
  );
}
